       ...
       <?php
        ...
        if($connection_database_status)
        {
            ...
            if($row=mysqli_fetch_array($query_status))
            {
                if($row['phone']==$phone)
                {
                    echo "<h1 style='color:#5cb85c;'>Recovered Password<h1><br>";
                    echo "<h1 style='color:#005960;'>".$row['password']."</h1>";
                }
            else
             {
                 echo "<h1 style='color:#db6851;'><br>Wrong Credentials.</h1>";
                 echo "<br><br><p><b><a href='forgetpassword.html' >Try to recover Again </a></b></p><br>";
             }
        }
        else
        {
